...
export const votePoll = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(pollId)) {
            return res.status(400).json({ success: false, message: "No poll found" })
        }
        ...
        if (poll.voters.some(({ userId: votersId }) => votersId.equals(userId))) {
            return res.status(400).json({ success: false, message: "You have already voted on this post" })
        }
        ...
        return res.status(201).json({ sucess: true, message: "Poll updated successfully" });
